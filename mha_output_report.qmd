...
# steps to reduce filesize of render
knitr::opts_chunk$set(dpi = 72)
##knitr::include_graphics("path/to/image.png", dpi = 72)

```

## **Introduction**

The analysis in this report was commissioned by the Care Quality Commission (CQC) to support a special interest report on the Mental Health Act (MHA) and children and young people (CYP). A separate report is being published alongside this that looks into the use of acute emergency services (ED and UCC) by CYP presenting with mental health issues.
...
All of the analysis in this report is sourced from the Mental Health Services Data Set (MHSDS). Data relating to MHA detention episodes are taken from MHS401 tables and all MH inpatient admissions from MHS501/MHS502. Administrative and demographic data were linked from the MHS001 (MPI) tables. As the MHSDS is a cumulative monthly refreshed dataset with multiple submissions of the same activity data was de-duplicated taking the latest and fullest submission of data for a particular detention episode or admission.
...
-   Most lengths of Mental Health Act detention spells are up to 1 day long with another (smaller) peak at around 28 days - aligning with expiry times specified in the various sections of the act.
...
-   There has been a small increase over time in mean distance CYP are admitted from home.
-   The median distance however is constant over time, suggesting that an increase in mainly the furthest distances are skewing the mean.
-   The population groups most affected (in relative terms) by increased distance from home over time are males (though females still travel much further), those aged 18-24, those in ‘other’ minority ethnic groups (though white patients typically travel slightly further) and in deprived populations (though less deprived patients tend to travel further).
-   The difference in mean travel distances from home between younger females (31km) and males (18km) is stark.
...
Across the 5 year period to 2023/24, there were `r comma(number_detentions)` detention spells in those aged under 25 at time of detention.

Of these, `r comma(number_single_episode_detentions)` were single episode detentions whereas `r comma(number_multi_episode_detentions)` had more than one detention episodes.
...
tar_read(conversion_map) |> 
  ...
  dplyr::rename("Section Conversions" = sections_all) |> 
...
This data refers to all completed MHA detention spells between 2019/20 and 2023/24 containing a conversion from Section 2 to Section 3. This could include detentions starting under any other section of the MHA and spells with other preceding or subsequent pattern of sections, including renewals of section 3.
...
A MHA detention spell can consist of consecutive MHA detentions so the length of stay in our use-case is defined as the difference in time between the start of the first MHA detention in a series to the discharge date of the last episode in a series.
...
The percentage of Mental Health Act (MHA) detentions with 1 or more re-detentions within 12 months is defined as the number of discharges from detention in any given period with at least 1 subsequent MHA detention spell admission dated within 365 days of the first spell discharge date.
...
Visually, for a small sample of patients this may look something like:
<br>
...
A re-admission is defined as a MH inpatient admission within 365 days of a MHA detention spell that did not start as a new MHA detention.

The percentage of MHA detentions that had 1 or more subsequent MH admission within 12 months has increased in 2022/23 compared to earlier years (@tbl-readmissions-number). This also varies by ICB (@fig-readmissions-mh).
...
When considering the total distance traveled by each subgroup over time, as shown in Figure @fig:Proportion_Total_DTT_Gender_Age, the split further emphasizes the disproportionate distance traveled by females, particularly in the younger age cohort (0–17). Over time, females have consistently accounted for a larger proportion of the total DTT for this age group. For example, in 2017/18, females in the 0–17 group made up 64% of the total distance traveled by that cohort, and this share has remained relatively high, with fluctuations between 62% and 66% through 2024/25 despite making up only 44-52% of the total number of admissions for the age group, depending on the year (see @fig:Proportion_Admissions_Gender_Age_Gap).

In contrast, the 18–24 group shows a more balanced split in total DTT between males and females. Initially, in 2017/18, females accounted for 52% of the total DTT in this age group, and while this percentage has remained fairly consistent, it has shifted slightly over time. The share of distance traveled by males has gradually increased, reaching nearly 50% by 2024/25.

These results highlight the gender disparities in travel burden, particularly for younger females, who consistently travel farther distances. While the 18–24 cohort shows a more balanced split, the proportion of DTT remains uneven in the younger cohort, underscoring a need to examine the causes and implications of this disparity more closely.

#### ![](Heather%20Scratch%20Space/Tableau%20Plots/Final%20fig%20for%20report%20age%20gender%20proportions%20avg%20DTT.png)
...
magick::image_read(tar_read(DTT_ICB_map_FY_2023_24))
...
magick::image_read(tar_read(DTT_ICB_map_FY_2024_25))
```
:::
:::::
...
magick::image_read(tar_read(DTT_ICB_map_female_2023_24))
```
:::
...
magick::image_read(tar_read(DTT_ICB_map_male_2023_24))
```
:::
:::::
...
magick::image_read(tar_read(DTT_ICB_map_0_17_2023_24))
```
:::
...
magick::image_read(tar_read(DTT_ICB_map_18_24_2023_24))
```
:::
:::::
...
magick::image_read(tar_read(DTT_ICB_map_white_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_black_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_asian_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_mixed_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_other_2023_24))
```
:::
...
magick::image_read(tar_read(DTT_ICB_map_most_2023_24))
```
:::
...
magick::image_read(tar_read(DTT_ICB_map_least_2023_24))
```
:::
:::::
...
magick::image_read(tar_read(DTT_ICB_map_1_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_2_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_3_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_4_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_5_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_6_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_7_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_8_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_9_2023_24))
```
...
magick::image_read(tar_read(DTT_ICB_map_10_2023_24))
```
:::
:::::::
::::::::::::::::::
...
##### % Change Avg DTT Table

![](Heather%20Scratch%20Space/Tableau%20Plots/Final%20fig%20for%20report%20age%20gender%20proportions%20avg%20DTT.png){#fig:Percent_Gender_Age_DTT_Table}

##### Proportion of Total DTT

![](Heather%20Scratch%20Space/Tableau%20Plots/Final%20Gender%20Age%20DTT%20Total%20Proportion.png){#fig:Proportion_Total_DTT_Gender_Age}

##### Gender/Age Gap by proportion of Total Admissions

![](Heather%20Scratch%20Space/Tableau%20Plots/Final%20Gender%20Age%20Admissions%20proportions.png){#fig:Proportion_Admissions_Gender_Age_Gap}
:::

...
Across all ethnic groups, admissions have decreased, while travel distances (both average and median) have generally increased. The Other Ethnicity cohort shows the most dramatic increase in travel distances, though the relatively small number of admissions, particularly in FY 2024/25, makes the data more susceptible to outliers.

-   White group: While this group maintains the largest share of admissions, it has experienced a notable drop, alongside a steady increase in travel distances.

-   Black group: Travel distances have risen moderately, though the group has seen a significant decrease in admissions.

-   Asian group: Both average and median travel distances have remained relatively stable, with only slight fluctuations, despite a sharp decline in admissions.

-   Mixed group: Travel distances have increased, even though the number of admissions has fallen significantly.

-   Other group: This group shows the largest increase in both average and median travel distances, despite the smallest number of admissions.
...
![](Heather Scratch Space/Tableau Plots/MM FY.png)
:::
...
![](Heather Scratch Space/Tableau Plots/Gender MM FY.png)
:::
...
![](Heather Scratch Space/Tableau Plots/Age MM FY.png)
:::
...
![](Heather Scratch Space/Tableau Plots/Ethnicity MM FY.png)
:::
...
![](Heather Scratch Space/Tableau Plots/IMD MM FY corrected.png)

##### Mean vs Median Quintiles

![](Heather Scratch Space/Tableau Plots/IMD MM FY corrected quintile.png)
:::
...
![](Heather Scratch Space/Tableau Plots/ICB MM FY.png)
:::
:::
...
![](Heather Scratch Space/Tableau Plots/IMD by age gender time corrected avg.png)

##### IMD: Mean and median trends over time by gender

![](Heather Scratch Space/Tableau Plots/IMD quintile gender corrected.png)
...
Of CYP MH spells, `r tar_read(honos_perc_worse) |> dplyr::pull(perc_better) |> janitor::round_half_up(2)`%  had a reduction in HONOS scores suggesting an improvement in symptoms (@fig-honos-histo) and `r tar_read(honos_perc_worse) |> dplyr::pull(perc_same) |> janitor::round_half_up(2)`%  had the same HONOS scores at the start and end of the spell. `r tar_read(honos_perc_worse) |> dplyr::pull(perc_worse) |> janitor::round_half_up(2)`% had an increase in HONOS scores suggesting a worsening of symptoms. However all spells with a relative change greater than 1 had low scores in the first HONOS assessment (@fig-honos-scatter) which could be due to data quality issues.
